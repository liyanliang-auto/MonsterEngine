...
void testTexturePool() {
    ...
    for (auto& tex : textures) {
        ...
        if (ptr) {
            ...
            // 写入测试图案
            memset(ptr, 0xEE, 1024);  // 只写�?KB作为测试
        } else {
    ...
    // 测试释放（注意：当前实现需要跟踪大小）
    // memSys.textureReleaseAll();  // 释放所�?
    MR_LOG_INFO("  �?Texture blocks remain allocated (use textureReleaseAll to clear)");
    }
...
void testHugePages() {
    ...
    if (available) {
        ...
        MR_LOG_INFO("  �?Huge pages not available on this system");
        MR_LOG_INFO("  �?Windows: Requires SeLockMemoryPrivilege");
        MR_LOG_INFO("  �?Linux: Check 'cat /proc/meminfo | grep HugePages'");
    }
    }
