...
// ============================================================================
// FForwardRenderer Implementation
// ============================================================================
...
void FForwardRenderer::Initialize()
{
    if (bInitialized)
    {
        MR_LOG(LogForwardRenderer, Warning, "Forward Renderer already initialized");
        return;
    }
    
    MR_LOG(LogForwardRenderer, Log, "Initializing Forward Renderer");
    
    // Create render passes
    DepthPrepass = new FDepthPrepass();
    OpaquePass = new FOpaquePass();
    SkyboxPass = new FSkyboxPass();
    TransparentPass = new FTransparentPass();
    ShadowDepthPass = new FShadowDepthPass();
    
    // Register passes with manager
    PassManager.RegisterPass(DepthPrepass);
    PassManager.RegisterPass(OpaquePass);
    PassManager.RegisterPass(SkyboxPass);
    PassManager.RegisterPass(TransparentPass);
    // Note: ShadowDepthPass is executed separately before main passes
    
    // Sort passes by priority
    PassManager.SortPasses();
    ...
    MR_LOG(LogForwardRenderer, Log, "Forward Renderer initialized with %d passes",
           PassManager.GetPasses().Num());
}
...
void FForwardRenderer::Shutdown()
{
    ...
    MR_LOG(LogForwardRenderer, Log, "Shutting down Forward Renderer");
    
    // PassManager will delete registered passes
    PassManager.ClearPasses();
    
    // Delete shadow pass (not registered with manager)
    if (ShadowDepthPass)
    {
        delete ShadowDepthPass;
        ShadowDepthPass = nullptr;
    }
    
    // Clear pointers (passes deleted by manager)
...
void FForwardRenderer::Render(FRenderPassContext& Context)
{
    if (!bInitialized)
    {
        MR_LOG(LogForwardRenderer, Warning, "Forward Renderer not initialized, cannot render");
        return;
    }
    
    MR_LOG(LogForwardRenderer, Verbose, "Forward Renderer: Beginning frame %u", Context.FrameNumber);
    
    // Step 1: Render shadow maps (if enabled)
    if (bShadowsEnabled)
    {
    ...
    // Step 2: Execute all registered passes in order
    // Order: DepthPrepass -> OpaquePass -> SkyboxPass -> TransparentPass
    PassManager.ExecuteAllPasses(Context);
    
    MR_LOG(LogForwardRenderer, Verbose, "Forward Renderer: Completed frame %u", Context.FrameNumber);
}

void FForwardRenderer::SetDepthPrepassEnabled(bool bEnable)
{
    bDepthPrepassEnabled = bEnable;
    
    if (DepthPrepass)
    {
        DepthPrepass->GetMutableConfig().bEnabled = bEnable;
        
        // Adjust opaque pass based on depth prepass state
        if (OpaquePass)
        {
            if (bEnable)
            {
                // Depth prepass enabled: opaque pass uses LessEqual, no depth clear
                OpaquePass->GetMutableConfig().bClearDepth = false;
                OpaquePass->GetMutableConfig().DepthStencilState.DepthCompareFunc = ECompareFunc::LessEqual;
            }
            else
            {
                // Depth prepass disabled: opaque pass clears and writes depth
                OpaquePass->GetMutableConfig().bClearDepth = true;
                OpaquePass->GetMutableConfig().DepthStencilState.DepthCompareFunc = ECompareFunc::Less;
            }
        }
    }
    
    MR_LOG(LogForwardRenderer, Log, "Depth prepass %s", bEnable ? "enabled" : "disabled");
}

void FForwardRenderer::SetSkyboxEnabled(bool bEnable)
{
    bSkyboxEnabled = bEnable;
    
    if (SkyboxPass)
    {
        SkyboxPass->GetMutableConfig().bEnabled = bEnable;
    }
    
    MR_LOG(LogForwardRenderer, Log, "Skybox %s", bEnable ? "enabled" : "disabled");
}

void FForwardRenderer::SetShadowsEnabled(bool bEnable)
{
    bShadowsEnabled = bEnable;
    
    MR_LOG(LogForwardRenderer, Log, "Shadows %s", bEnable ? "enabled" : "disabled");
}

void FForwardRenderer::SetShadowConfig(const FShadowMapConfig& Config)
{
    ShadowConfig = Config;
    
    if (ShadowDepthPass)
    {
        ShadowDepthPass->SetShadowConfig(Config);
    }
    
    MR_LOG(LogForwardRenderer, Log, "Shadow config updated: Resolution=%d, Type=%d, Cascades=%d",
           Config.Resolution, static_cast<int32>(Config.Type), Config.NumCascades);
}

void FForwardRenderer::RenderShadowMaps(FRenderPassContext& Context)
{
    if (!ShadowDepthPass || !Context.VisibleLights)
    {
        return;
    }
    
    // Clear previous shadow data
    ShadowDataArray.Reset();
    
    // Gather shadow-casting lights
    TArray<FLightSceneInfo*> ShadowCastingLights;
    GatherShadowCastingLights(Context, ShadowCastingLights);
    ...
    MR_LOG(LogForwardRenderer, Verbose, "Rendering shadow maps for %d lights", 
           ShadowCastingLights.Num());
    
    // Render shadow map for each shadow-casting light
    for (FLightSceneInfo* Light : ShadowCastingLights)
    {
        ShadowDepthPass->SetLight(Light);
        ShadowDepthPass->SetShadowConfig(ShadowConfig);
        
        // For CSM, render each cascade
        if (ShadowConfig.Type == EShadowMapType::Cascaded)
        {
            for (int32 CascadeIndex = 0; CascadeIndex < ShadowConfig.NumCascades; ++CascadeIndex)
            {
                ShadowDepthPass->SetCascadeIndex(CascadeIndex);
        ...
        ShadowDataArray.Add(ShadowDepthPass->GetShadowData());
    }
}

void FForwardRenderer::GatherShadowCastingLights(FRenderPassContext& Context, TArray<FLightSceneInfo*>& OutLights)
{
    if (!Context.VisibleLights)
    {
        return;
    }
