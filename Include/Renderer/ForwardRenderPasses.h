...
/**
 * Complete forward rendering pipeline
 * Manages and executes all forward rendering passes in order
 */
...
public:
     ...
     * Creates and registers all default passes
     */
    void Initialize();
    
    /**
     * Shutdown the forward renderer
     * Cleans up all passes
     */
    void Shutdown();
    
    /**
     * Render a frame
     * @param Context - Render context with scene and view info
     */
    void Render(FRenderPassContext& Context);
    
    /**
     * Get the pass manager
     * @return Reference to the pass manager
     */
    FRenderPassManager& GetPassManager() { return PassManager; }
    
    /**
     * Get a specific pass
     * @param PassType - Type of pass to get
     * @return Pointer to the pass, or nullptr
     */
    template<typename T>
    T* GetPass(ERenderPassType PassType)
    {
        return static_cast<T*>(PassManager.GetPass(PassType));
    }
     ...
     * @param bEnable - True to enable
     */
    void SetDepthPrepassEnabled(bool bEnable);
    
    /**
     * Check if depth prepass is enabled
     * @return True if enabled
     */
    bool IsDepthPrepassEnabled() const { return bDepthPrepassEnabled; }
    
    /**
     * Enable/disable skybox rendering
     * @param bEnable - True to enable
     */
    void SetSkyboxEnabled(bool bEnable);
    
    /**
     * Check if skybox is enabled
     * @return True if enabled
     */
    bool IsSkyboxEnabled() const { return bSkyboxEnabled; }
     ...
     */
    void SetShadowConfig(const FShadowMapConfig& Config);
...
protected:
     ...
     * @param Context - Render context
     */
    void RenderShadowMaps(FRenderPassContext& Context);
     ...
     */
    void GatherShadowCastingLights(FRenderPassContext& Context, TArray<FLightSceneInfo*>& OutLights);
    
private:
    /** Pass manager */
    FRenderPassManager PassManager;
    
    /** Depth prepass instance */
    FDepthPrepass* DepthPrepass = nullptr;
    
    /** Opaque pass instance */
    FOpaquePass* OpaquePass = nullptr;
    
    /** Skybox pass instance */
    FSkyboxPass* SkyboxPass = nullptr;
    
    /** Transparent pass instance */
    FTransparentPass* TransparentPass = nullptr;
    
    /** Shadow depth pass instance */
    FShadowDepthPass* ShadowDepthPass = nullptr;
    ...
    bool bDepthPrepassEnabled = true;
